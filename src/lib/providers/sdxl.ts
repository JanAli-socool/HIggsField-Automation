...
const SDXL_BASE_URL = "https://api.replicate.com/v1";
const SDXL_MODEL_VERSION = "stability-ai/sdxl:39ed52f2a78e934b3ba6e2a89f5b1c712de7dfea535525255b1aa35c5565e08b";

export class SDXLProvider extends BaseProvider implements GenerationProvider {
  readonly name = "SDXLProvider";
  readonly supportedMediaTypes: GenerationParameters["media_type"][] = ["image", "image_to_image", "image_edit", "inpainting"];
  readonly supportedModels: GenerationParameters["model"][] = ["sdxl"];
  readonly maxDuration = 60;
  readonly maxResolution = { width: 1024, height: 1024 };

  constructor(config: ProviderConfig) {
    super({ ...config, baseUrl: config.baseUrl || SDXL_BASE_URL });
  }
  ...
  async validate(request: GenerationRequest): Promise<ValidationResult> {
    ...
    if (["image_to_image", "image_edit", "inpainting"].includes(parameters.media_type)) {
  ...
  async submit(request: GenerationRequest): Promise<GenerationJob> {
    ...
    if (parameters.media_type === "image_to_image" || parameters.media_type === "image_edit") {
      input.image = parameters.input_image_url;
      input.strength = parameters.strength || 0.75;
    }

    if (parameters.media_type === "inpainting") {
      input.image = parameters.input_image_url;
      input.mask = parameters.mask_url;
      input.strength = parameters.strength || 0.99;
    }

    const response = await this.makeRequest<SDXLGenerateResponse>("/predictions", {
      method: "POST",
      body: JSON.stringify({
  ...
  async getStatus(jobId: string): Promise<{ status: JobStatus; progress: number; resultUrls?: string[]; error?: string; failureType?: FailureType }> {
    ...
    try {
      const response = await this.makeRequest<SDXLStatusResponse>(`/predictions/${providerJobId}`);
